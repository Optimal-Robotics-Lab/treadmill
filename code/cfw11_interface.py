...
from treadmill_status.msg import TreadmillStatus
import glob
...
class Treadmill(Node):
    ...
    def _read_logic_status(self):
        ...
        logic_msg =  self.client.read_holding_registers(address=param, count=1, slave=1)
    ...
    def _update_speed(self):
        self.rpm = self.read_param("motor_speed")
        self.speed_mps = self.rpm * RPM_TO_MPS
    ...
    def update_loop(self):
        ...
        self._update_speed()
        # get logic status
        self._read_logic_status()
        
        # if fault or alarm present , read and publish info
        if self.faulted or self.alarm:
            # fault message
            if self.faulted:
                ...
                print(fault_info)
                self.error_code = f"F{present_fault}"
                self.faulted = False # clear alarm flag if fault is present to avoid duplicate messages
            ...
            if self.alarm:
                ...
                print(alarm_info)
                self.error_code = f"A{present_alarm}"
                self.alarm = False # clear alarm flag after reporting
    ...
    def on_start(self):
        ...
        # set control to vector control mode
